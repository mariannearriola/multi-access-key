...
console.log(tagger.tag(["switch", "plants"]));


var dataLinks = require('./dataset_links.json');

function scrapeIt(url, int) {
    //console.log(int);
    var retArr = [];
    // SCRAPE KEY
    request(url, (error, response, html) => {
        if(!error && response.statusCode == 200) {
            ...
            $('p').each((i, el) => {
                ...
                if($(el).children('span.itemsubheading').text().includes("Illustrations.")) {
                    return this;
                }
                            ...
                            for(var c = 0; c < splitCom.length; c++) {
                                ...
                                if(attr == "") {
                                    ...
                                    if(wordArr.length == 2 && (attr == "")) {
                                        ...
                                        if((findTokens[1].includes("\"tag\":\"JJ") || findTokens[1].includes("\"tag\":\"N")) && findTokens[2].includes("\"tag\":\"N")) {
                                        ...
                                        if(vals[vals.length-1] && vals[vals.length-1].length == 0) { continue; }
                                        newTrait += fix(attr);
                                        newTrait += "-";
                                        newTrait += fix(vals[vals.length-1]);
                                        if(newTrait.charAt(newTrait.length-1) == "-") { newTrait += "null"; }
                                        newTrait += ",";
                                        if((findTokens[1].includes("\"tag\":\"JJ") || findTokens[1].includes("\"tag\":\"N")) && findTokens[2].includes("\"tag\":\"N")) {
                                                attr = "";
                                            }
                                    ...
                                    // MORE THAN TWO WORDS AND ATTR EMPTY
                                        ...
                                        for(var ft = findTokens.length-1; ft > 0; ft--) {
                                            ...
                                            // VERB/PREPOSITION/- CONNECTED WORDS (not adverb)
                                            if(((findTokens[ft].includes("\"tag\":\"V") && !(findTokens[ft].includes("\"tag\":\"VBN") && !(findTokens[ft].includes("\"tag\":\"RB")))) || findTokens[ft].includes("\"tag\":\"IN")) && findCommas.charAt(findCommas.indexOf(tok)-1) != "-") {
                                                var valStart;
                                                if(findTokens[ft].includes("\"tag\":\"IN")) {
                                                    valStart = findTokens[ft-1].substring(0,findTokens[ft-1].indexOf("\""));
                                                    ...
                                                    if(ft == 1) {
                                                        ...
                                                        valStart = findTokens[ft].substring(0,findTokens[ft].indexOf("\""));
                                                    }
                                            ...
                                            if(findTokens[ft].includes("\"tag\":\"VBN") || findTokens[ft].includes("\"tag\":\"RB")) {
                                                while(findTokens[ft-1].includes("\"tag\":\"VBN") || findTokens[ft-1].includes("\"tag\":\"RB")) { ft--; }
                                                var valStart = findTokens[ft-1].substring(0,findTokens[ft-1].indexOf("\""));
                                                attr = findCommas.substring(0,findCommas.indexOf(valStart)+valStart.length);
                                                vals.push(findCommas.substring(findCommas.indexOf(valStart)+valStart.length));
                                                newTrait += fix(attr);
                                                newTrait += "-",
                                            ...
                                            if(ft == 1) {
                                                for(var to = ft; to < findTokens.length-1; to++) {
                                                    if(findTokens[ft].includes("\"tag\":\"VBN") || findTokens[ft].includes("\"tag\":\"RB")) {
                                                        var valStart = findTokens[ft-1].substring(0,findTokens[ft].indexOf("\""));
                                                        attr = findCommas.substring(0,findCommas.indexOf(valStart)+valStart.length);
                                                        vals.push(findCommas.substring(findCommas.indexOf(valStart)+valStart.length));
                                                        newTrait += fix(attr);
                                                        newTrait += "-",
                                ...
                                // ATTR IS NOT EMPTY
                                    ...
                                    if(findCommas && findCommas.length == 0) { continue; }
                                    newTrait += fix(attr);
                                    newTrait += "-";
